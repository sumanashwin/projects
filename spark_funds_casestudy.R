...
eng_countries <-
  read.csv("english_countries.txt",
           sep = "\t",
...
unique_companies_rounds2 <-
  unique(tolower(rounds2$company_permalink))
...
setdiff(tolower(rounds2$company_permalink),
        tolower(companies$permalink)) #To show the difference between 2 columns
#character(0)
...
master_frame <-
  ...
  )  #Doing innerjoingjoin which is under the dplyr package
...
investment_summary <-
  filter(
    master_frame,
    funding_round_type %in% c("angel", "seed", "private_equity", "venture"),
    raised_amount_usd != -1
  ) %>% group_by(funding_round_type) %>% summarise(average_funding_amount =
...
master_frame1$country_names <-
  countrycode(master_frame1$country_code, "iso3c", "country.name")

#################################################################################################
#Getting the countries with engish speaking ones

country_vect <-
  c(eng_countries$Country)  #creating a vector of all english speaking countries,dataframe created from imported txt file
#This is useful for plotting the top 9 countries in Tableau

master_frame1$"english_speaking_countries" <-
  grepl(paste(country_vect, collapse = "*|"),
        master_frame1$country_names)
...
top9 <-
  group_by(master_frame1, country_code)  %>% filter(funding_round_type %in% "venture",
                                                    country_code != "",
                                                    raised_amount_usd != -1) %>%  summarise(total_investment = sum(raised_amount_usd)) %>% arrange(desc(total_investment)) %>% slice(1:9)

#appending country names

top9$country_names <-
  countrycode(top9$country_code, "iso3c", "country.name")

#Determining english speaking or not for Top9 countries

top9$"english_speaking_countries" <-
  grepl(paste(country_vect, collapse = "*|"), top9$country_names)
...
top3 <-
  filter(top9, english_speaking_countries == TRUE) %>% slice(1:3)
...
master_frame1 <-
  separate(master_frame1,
           category_list,
           into = c("primary_sector"),
           sep = "\\|") #Using the seperate function from tidyr package
...
mapping_long$category_list <-
  str_replace_all(mapping_long$category_list, pattern = "0", "na")  #Replacing the '0' in the string to 'na'
...
sector_analysis <- function(x) {
  countrywise_sector_df <-
    filter(
      merge_frame2,
      country_code %in% x,
      funding_round_type %in% "venture",
      between(raised_amount_usd, 5000000, 15000000),
      main_Sector != "Blanks",!is.na(main_Sector)
    ) %>%
    group_by(main_Sector) %>% mutate(
      no_of_investments = n(),
...
investment_analysis <- function(x) {
  ...
  sector_numbers <-
    group_by(x, main_Sector) %>% summarise(no_of_investments = n(),
                                           total_investment = sum(total_investment)) #Rolling up by main sector and aggregating by sum of inv and count of Inv
  
  total_num_inv <-
    sum(sector_numbers$no_of_investments) #Total number of investments across main sectors
  total_sum_inv <-
    sum(sector_numbers$total_investment)  #Total sum of investment across all main sectors
  
  
  D4 <-
    group_by(x, main_Sector) %>% summarise(n = n()) %>% arrange(desc(n)) %>% slice(1:3) #grouping  mainsector and getting the top3 investments by number
  mainsectors <- unlist(D4[1])
  no_of_inv <- unlist(D4[2])
  
  company1 <-
    filter(x, main_Sector %in% mainsectors[1]) %>% group_by(name)   %>% summarise(n =
                                                                                    sum(raised_amount_usd)) %>% arrange(desc(n)) %>% slice(1:1)
  top_company1 <-
    unlist(company1[1])   #getting the company wise data for the Top sector and amount of funding raised
  
  company2  <-
    filter(x, main_Sector %in% mainsectors[2]) %>%  group_by(name)  %>%  summarise(n =
                                                                                     sum(raised_amount_usd))  %>% arrange(desc(n)) %>% slice(1:1)
  top_company2 <-
    unlist(company2[1])    # #getting the company wise data for the second  Top sector and amount of funding raised
...
D1a <-
  investment_analysis(D1) # Passing the D1 dataframe as object to the function
D2a <-
  investment_analysis(D2) # Passing the D2 dataframe as object to the function
D3a <-
  investment_analysis(D3)  # Passing the D3 dataframe as object to the function
investment_summary_dataframe <-
  bind_rows(D1a, D2a, D3a)  #Binding all the data frame row wise to get one composite dataframe
...
row.names(investment_summary_dataframe)[2] <-
  "Country2(Great Britain and Ireland)"
